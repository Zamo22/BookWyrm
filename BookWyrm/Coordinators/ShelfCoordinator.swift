...
class ShelfCoordinator: NSObject, Coordinator, UINavigationControllerDelegate {
    ...
    func start() {
        ...
        let vc = PlainShelfController.instantiate()
        guard let image = UIImage(named: "shelf") else {
            return
        }
        vc.tabBarItem = UITabBarItem(title: "Bookshelf", image: image, tag: 0)
        vc.coordinator = self
